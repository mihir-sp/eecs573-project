...
static FILE* ppfile0 = NULL;
static FILE* ppfile1 = NULL;


void open_pipeline_output_file(char* file_name0, char* file_name1)
{
    printf("INSIDE OPEN PIPELINE FILES: %s, %s", file_name0, file_name1);
    if (ppfile0 == NULL)
        ppfile0 = fopen(file_name0, "w");

    if (ppfile1 == NULL)
        ppfile1 = fopen(file_name1, "w");
}

void testPrint()
{
    FILE* f = fopen("testfile.out", "w");
    fprintf(f, "HELLO\n");
    fclose(f);

}

void print_header(char* str, bool toFile0)
{
    if (toFile0)
    {
        // print to file 1
        if (ppfile0 != NULL)
            fprintf(ppfile0, "%s", str);
    }
    else
    {
        // print to file 2
        if (ppfile1 != NULL)
            fprintf(ppfile1, "%s", str);
    }
}



void print_cycles(bool toFile0)
{
    if (toFile0)
    {
        if (ppfile0 != NULL)
...
void print_stage(char* div, int inst, int npc, int valid_inst, bool toFile0)
{
    FILE* ppfile = toFile0 ? ppfile0 : ppfile1;
...
void print_reg(int wb_reg_wr_data_out_hi, int wb_reg_wr_data_out_lo,
               int wb_reg_wr_idx_out, int wb_reg_wr_en_out, bool toFile0)
{
    FILE* ppfile = toFile0 ? ppfile0 : ppfile1;
    if (ppfile == NULL)
        return;
    ...
    if (wb_reg_wr_en_out)
        if ( (wb_reg_wr_data_out_hi == 0) ||
            ...
            fprintf(ppfile, "r%d=%u  ", wb_reg_wr_idx_out, wb_reg_wr_data_out_lo);
        else
...
void print_membus(int proc2mem_command, int mem2proc_response,
                  ...
                  int proc2mem_data_hi, int proc2mem_data_lo, bool toFile0)
{
    FILE* ppfile = toFile0 ? ppfile0 : ppfile1;
    if (ppfile == NULL)
        return;
